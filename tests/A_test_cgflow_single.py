...
def main(
    ...
    with torch.autocast("cuda", enabled=True, dtype=torch.bfloat16):
        for batch in tqdm(dataloader, total=len(dataloader.dataset), desc="Evaluating"):
            ...
            w1 = Chem.SDWriter(str(save_dir / "sample_true.sdf"))
            w2 = Chem.SDWriter(str(save_dir / "sample_state.sdf"))
            w3 = Chem.SDWriter(str(save_dir / "sample_pred.sdf"))
            ...
            w2.close()
            w3.close()
            natoms = set([mol.GetNumAtoms() for mol, _ in traj_mols])
            print(sorted(list(natoms)))
            breakpoint()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_dir", type=str, default="./result/eval_single/")
