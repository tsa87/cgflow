...
class CoordNorm(torch.nn.Module):
    ...
    This layer allows 4 different types of coordinate normalisation (defined in the norm argument):
        ...
        4. 'off' - No normalisation is applied
    ...
    def __init__(self, n_coord_sets, norm="length", eps=1e-6):
        ...
        if norm not in ["none", "gvp", "length", "off"]:
    ...
    def _norm_params(self, coord_sets, node_mask):
        # Compute the centre of mass and zero the coordinates
        com = smolF.calc_com(coord_sets, node_mask)
        shifted_coords = coord_sets - com
        shifted_coords *= node_mask.unsqueeze(-1)

        n_atoms = node_mask.sum(dim=-1, keepdim=True)
        lengths = torch.linalg.vector_norm(shifted_coords, dim=-1)
        ...
        if self.norm == "length":
            ...
            raise ValueError(f"Unknown normalisation type '{self.norm}'")

        return com, coord_div

    def _apply_cord_norm(self, coord_sets, node_mask, com, coord_div):
        out_coord_sets = (coord_sets - com) * node_mask.unsqueeze(-1)
        out_coord_sets = (out_coord_sets * self.set_weights) / coord_div
        out_coord_sets = out_coord_sets * node_mask.unsqueeze(-1)
        return out_coord_sets
    ...
    def forward(self, coord_sets, node_mask):
        ...
        """
        if self.norm == "off":
            return coord_sets * node_mask.unsqueeze(-1)

        com, coord_div = self._norm_params(coord_sets, node_mask)
        out_coord_sets = self._apply_cord_norm(coord_sets, node_mask, com, coord_div)
        return out_coord_sets
...
class ComplexCoordNorm(CoordNorm):

    def forward(self, ligand_coord_sets, ligand_mask, protein_coord_sets=None, protein_mask=None):
        """Apply coordinate normlisation layer

        Args:
            ligand_coord_sets (torch.Tensor): Coordinate tensor, shape [batch_size, n_sets, n_lig_nodes, 3]
            ligand_mask (torch.Tensor): Mask for nodes, shape [batch_size, n_sets, n_lig_nodes], 1 for real, 0 otherwise
            protein_coord_sets (torch.Tensor): Coordinate tensor, shape [batch_size, n_sets, n_prot_nodes, 3]
            protein_mask (torch.Tensor): Mask for nodes, shape [batch_size, n_sets, n_prot_nodes], 1 for real, 0 otherwise

        Returns:
            torch.Tensor: Normalised coords, shape [batch_size, n_sets, n_nodes, 3]
        """
        if self.norm == "off":
            return ligand_coord_sets * ligand_mask.unsqueeze(-1)

        com, coord_div = super()._norm_params(ligand_coord_sets, ligand_mask)
        norm_ligand_coord_sets = super()._apply_cord_norm(ligand_coord_sets, ligand_mask, com, coord_div)
        if protein_coord_sets is not None:
            norm_protein_coord_sets = super()._apply_cord_norm(protein_coord_sets, protein_mask, com, coord_div)
            return norm_ligand_coord_sets, norm_protein_coord_sets
        return norm_ligand_coord_sets
...
class EdgeMessages(torch.nn.Module):
    def __init__(
        ...
        self.coord_norm = CoordNorm(n_coord_sets, norm="off" if no_coord_norm else "none")
...
class NodeAttention(torch.nn.Module):
    ...
    def forward(self, node_feats, messages, adj_matrix):
        ...
        Args:
            node_feats (torch.Tensor): Node feature tensor, shape [batch_size, n_in_nodes, d_model]
            messages (torch.Tensor): Messages tensor, shape [batch_size, n_out_nodes, n_in_nodes, d_message]
            adj_matrix (torch.Tensor): Adjacency matrix, shape [batch_size, n_out_nodes, n_in_nodes]

        Returns:
            torch.Tensor: Accumulated node features, shape [batch_size, n_out_nodes, d_model]
        """
...
class BondRefine(torch.nn.Module):
    ...
    def __init__(self, d_model, d_message, d_edge, d_ff=None, no_coord_norm=False):
        super().__init__()

        d_ff = d_message if d_ff is None else d_ff
        in_feats = (2 * d_message) + d_edge + 2

        self.coord_norm = CoordNorm(1, norm="off" if no_coord_norm else "none")
        self.node_norm = torch.nn.LayerNorm(d_model)
...
class EquiInvDynamics(torch.nn.Module):
    ...
    def __init__(
        ...
        if self.bond_refine:
            self.refine_layer = BondRefine(d_model, d_message, d_edge, no_coord_norm=coord_norm == "off")
...
class SemlaGenerator(MolecularGenerator):
    ...
    def forward_api(
        ...
        atom_feats = atom_feats * atom_mask.unsqueeze(dim=-1)
        pred_coords = smolF.zero_com(pred_coords, node_mask=atom_mask)
        pred_coords = pred_coords * atom_mask.unsqueeze(-1)
        return pred_coords, atom_feats
