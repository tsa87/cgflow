...
GEOM_DRUGS_BUCKET_LIMITS = [24, 28, 32, 36, 40, 44, 48, 52, 56, 60, 64, 72, 96, 192, 384]
...
# Applies the following transformations to a pocket ligand complex:
# 1. Scales coordinate values by 1 / coord_std (so that they are standard normal)
# 2. Applies a random rotation to the coordinates
# 3. Removes the centre of mass of the holo pocket from the complex
# 4. Creates a one-hot vector for the atomic numbers of each atom in the ligand
# 5. Creates a one-hot vector for the bond type for every possible bond in the ligand
# 6. Encodes charges as non-negative numbers according to encoding map
# 7. Transform holo into a geometric mol object and perform step 4-6
# 8. Optionally, extract only the C_alpha atoms from the protein pocket
# 9. Scale the holo (not holo mol) back to original scale - we need this for computing PoseCheck metrics
def complex_transform(complex, vocab, n_bonds, coord_std, fix_pos=False, c_alpha_only=False):
    if not fix_pos:
        rotation = tuple(np.random.rand(3) * np.pi * 2)
        complex = complex.scale(1.0 / coord_std).rotate(rotation).zero_holo_com()

    transformed_ligand = complex.ligand._copy_with(
        atomics=smolF.one_hot_encode_atomics(complex.ligand.atomics, vocab),
        bond_types=smolF.one_hot_encode_tensor(complex.ligand.bond_types, n_bonds),
        charges=smolF.charge_to_index(complex.ligand.charges),
    )

    holo = complex.holo.select_c_alpha_atoms() if c_alpha_only else complex.holo.copy()
    transformed_holo_mol = holo.to_geometric_mol()

    # One-hot encode either the C_alpha atoms or all atoms in the holo
    if c_alpha_only:
        res_names = holo.atoms.res_name
        atomics = smolF.one_hot_encode_tensor(smolF.aa_to_index(res_names), len(smolRD.IDX_RESIDUE_MAP))
    else:
        atomics = smolF.one_hot_encode_atomics(transformed_holo_mol.atomics, vocab)

    transformed_holo_mol = transformed_holo_mol._copy_with(
        atomics=atomics,
        bond_types=smolF.one_hot_encode_tensor(transformed_holo_mol.bond_types, n_bonds),
        charges=smolF.charge_to_index(transformed_holo_mol.charges),
    )

    # Convert the holo back to original scale
    unscaled_holo = complex.holo.scale(coord_std)

    return complex.copy_with(ligand=transformed_ligand, holo=unscaled_holo, holo_mol=transformed_holo_mol)
...
def init_metrics(model=None, data_path=None):
