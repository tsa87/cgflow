...
from semlaflow.util.rdkit import PeriodicTable
...
def _check_type(obj, obj_types, name="object"):
    if not isinstance(obj, obj_types):
        raise TypeError(f"{name} must be an instance of {obj_types} or one of its subclasses, got {type(obj)}")
...
class ResidueFilterSelect(Select):
    ...
    def accept_residue(self, residue):
        ...
        if not is_complete and self.verbose:
            print(f"Residue {residue.get_resname()} {residue.get_id()} is missing atoms {required_atoms - atom_names}")
    ...
    def accept_atom(self, atom):
        ...
        if atom_name in self.seen_atoms[residue_id]:
            if self.verbose:
                print(f"Duplicate atom {atom_name} in residue {residue.get_resname()} {residue.get_id()}")
...
class ProteinPocket:
    def __init__(self, atoms: AtomArray, bonds: BondList, str_id: str | None = None):
    ...
    def res_length(self) -> int:
        return len(
            set(
                [
                    str(res_id) + chain_id
                    for res_id, chain_id in zip(self.atoms.res_id, self.atoms.chain_id, strict=False)
                ]
            )
        )
    ...
    def bonds_tensor(self) -> _T:
        ...
        bonds[:, 2][bonds[:, 2] >= BIOTITE_AROMATIC_BOND_START_IDX] = aromatic_bond_idx
        return bonds
    ...
    # *** Subset functions ***
    def select_atoms(self, mask: np.ndarray, str_id: str | None = None) -> ProteinPocket:
    ...
    def shift(self, shift: tuple[float, float, float] | np.ndarray) -> ProteinPocket:
    ...
    @staticmethod
    def from_pocket_atoms(atoms: AtomArray, infer_res_bonds: bool = False) -> ProteinPocket:
    ...
    @staticmethod
    def from_pdb(pdb_file: str | Path, infer_res_bonds: bool = False, sanitize: bool = True) -> ProteinPocket:
        if sanitize:
        ...
        return ProteinPocket.from_pocket_atoms(atoms, infer_res_bonds=infer_res_bonds)
    ...
    def write_cif(self, filepath: str | Path, include_bonds: bool = False) -> None:
        if include_bonds:
    ...
    def write_pdb(self, filepath: str | Path, include_bonds: bool = False) -> None:
    ...
    def copy_with(self, atoms: AtomArray | None = None, bonds: BondList | None = None) -> ProteinPocket:
...
class PocketComplex:
    def __init__(
        ...
        holo: ProteinPocket,
        ligand: GeometricMol,
        holo_mol: GeometricMol | None = None,  # GeometricMol representation of holo pocket
    ...
    def from_bytes(data: bytes) -> PocketComplex:
        ...
        apo = ProteinPocket.from_bytes(obj["apo"]) if obj.get("apo") is not None else None
        interactions = obj.get("interactions")

        system = PocketComplex(holo, ligand, apo=apo, interactions=interactions, metadata=obj["metadata"])
        return system
    ...
    def remove_hs(self, include_ligand: bool = False) -> PocketComplex:
        ...
        if include_ligand:
            raise NotImplementedError("Removing Hs from the ligand is not supported.")

        protein_atom_mask = self.holo.atoms.element != "H"
        holo_subset = self.holo.select_atoms(protein_atom_mask)

        apo_subset = self.apo.select_atoms(protein_atom_mask) if self.apo is not None else None
        interactions_subset = self.interactions[protein_atom_mask, :, :] if self.interactions is not None else None
    ...
    def copy_with(
        ...
        if holo_mol is None:
            holo_mol_copy = self.holo_mol.copy() if self.holo_mol is not None else None
        else:
        ...
        if interactions is None:
            interactions_copy = np.copy(self.interactions) if self.interactions is not None else None
        else:
    ...
    def _check_holo_apo_match(holo: ProteinPocket, apo: ProteinPocket):
        ...
        # Check sizes match
        if len(holo) != len(apo):
            raise ValueError(f"Apo and holo must have the same number of atoms, got {len(apo)} and {len(holo)}")
    ...
    def rotate(self, rotation: TCoord) -> PocketComplex:
        ...
        return self.copy_with(
            ...
            apo=self.apo.rotate(rotation_arr) if self.apo is not None else None,
            interactions=self.interactions,
        )
...
class PocketComplexBatch(Sequence):
    ...
    def remove_hs(self, include_ligand: bool | None = False):
        subset_systems = [system.remove_hs(include_ligand=include_ligand) for system in self._systems]
