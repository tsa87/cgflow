...
from .rule_semla import SemlaRule


data_folder = Path(__file__).parent / "data"
data = OmegaConf.load(data_folder / "reaction_semla.yaml")
RULES = [SemlaRule(info) for info in data]
...
def find_rxn_bonds(mol: RDMol) -> list[tuple[tuple[int, int], tuple[str, str], int]]:
    ...
    for idx, rule in enumerate(RULES):
        pattern_matches: list[tuple[int, ...]] = mol.GetSubstructMatches(
