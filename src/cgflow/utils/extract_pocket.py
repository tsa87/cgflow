...
def get_mol_coords(mol_path: str | Path) -> np.ndarray:
    format = Path(mol_path).suffix[1:]
    pbmol: pybel.Molecule = next(pybel.readfile(format, str(mol_path)))
...
class LigandSelect(Select):
    ...
    def accept_residue(self, residue):
        ...
        if self.use_calpha:
            ...
            pairwise_dis = np.linalg.norm(
                residue_positions.reshape(1, -1, 3) - self.coords.reshape(-1, 1, 3),
                axis=-1,
            )
...
def extract_pocket_from_ref_ligand(
    ...
    if (not out_pocket_path.exists()) or force_pocket_extract:
        ...
        io.save(
            str(out_pocket_path),
            LigandSelect(ref_lig_coords, cutoff, use_calpha=use_calpha),
        )


def extract_pocket_crossdocked_v0(
    protein_path: str | Path,
    ref_ligand_path: str | Path,
    ...
    if out_pocket_path is None:
        ...
        out_pocket_path = protein_path.parent / (
            protein_path.stem + "_" + ref_ligand_path.stem + "_crossdocked_v0.pdb"
        )
    extract_pocket_from_ref_ligand(
        protein_path,
        ref_ligand_path,
        out_pocket_path,
        10.0,
        False,
        force_pocket_extract,
    )
    return Path(out_pocket_path)


def extract_pocket_crossdocked_v1(
    protein_path: str | Path,
    ref_ligand_path: str | Path,
    out_pocket_path: str | Path | None = None,
    force_pocket_extract: bool = False,
    ...
    if out_pocket_path is None:
        protein_path, ref_ligand_path = Path(protein_path), Path(ref_ligand_path)
        out_pocket_path = protein_path.parent / (
            protein_path.stem + "_" + ref_ligand_path.stem + "_crossdocked_v1.pdb"
        )
    extract_pocket_from_ref_ligand(
        protein_path, ref_ligand_path, out_pocket_path, 10.0, True, force_pocket_extract
    )
    return Path(out_pocket_path)
...
def extract_pocket_plinder(
    ...
    if out_pocket_path is None:
        protein_path, ref_ligand_path = Path(protein_path), Path(ref_ligand_path)
        out_pocket_path = protein_path.parent / (
            protein_path.stem + "_" + ref_ligand_path.stem + "_plinder.pdb"
        )
    extract_pocket_from_ref_ligand(
        protein_path, ref_ligand_path, out_pocket_path, 6.0, False, force_pocket_extract
    )
    return Path(out_pocket_path)
