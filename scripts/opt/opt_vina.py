...
def parse_args() -> DictConfig:
    ...
    parser.add_argument("--exhaustiveness", type=int, help="Exhaustiveness of Vina docking (only Redock).")

    # pose refine
    parser.add_argument(
        "--refine", type=str, choices=["local_opt", "redock"], help="Refine the generated pose with Vina."
    )
    parser.add_argument("--ff_opt", type=str, choices=["none", "uff", "mmff"], help="Force field for optimization.")
...
if __name__ == "__main__":
    from tasks.autodock_vina import AutoDockVinaMOOTrainer
    ...
    # vina-qed moo
    config.desc = "Multi objective optimization for AutoDock Vina with QED"
    ...
    config.task.docking.exhaustiveness = param.exhaustiveness
    ...
    # pose refining
    config.task.docking.redocking = param.refine == "redock"
    config.task.docking.ff_opt = param.ff_opt
    ...
    trainer = AutoDockVinaMOOTrainer(config)
    trainer.run()
